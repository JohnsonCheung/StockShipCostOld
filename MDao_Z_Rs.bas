...
Function RsDry(A As DAO.Recordset, Optional InclFldNm As Boolean) As Variant()
...
RsIsBrk = Not AyIsEq(RsKyDr(A, GpKy), LasVy)
End Function
...
FdsSetSqRow A.Fields, Sq, R, NoTxtSngQ
End Sub

Function RsSq(A As DAO.Recordset, Optional InclFldNm As Boolean) As Variant()
RsSq = DrySq(RsDry(A, InclFldNm))
End Function
...
Property Let RsFdVal(A As DAO.Recordset, F, V)
With A
    .Edit
    .Fields(F).Value = V
    .Update
...
Property Get RsFdVal(A As DAO.Recordset, F)
With A
    If .EOF Then Exit Property
    If .BOF Then Exit Property
    RsFdVal = .Fields(F).Value
End With
End Property
