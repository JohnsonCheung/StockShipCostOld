...
Next
End Function
Private Function ErMsg_T_VBarCnt$(A As Lnx, VBarCnt%)
ErMsg_T_VBarCnt = ErMsg(A.Ix, "There should be 0 or 2 VBar")
End Function
Private Function BrkTLin(T As Lnx, OEr$()) As T
Select Case SubStrCnt(T.Lin, "|")
Case 0, 2
Case Else
    PushI OEr, ErMsg_T_VBarCnt(T, SubStrCnt(T.Lin, "|"))
End Select
...
Dim LikFF1$, V$, Ty$, Ay(), Brk$(), Rest$(), L$
L = ELin.Lin
With BrkELin
    AyAsg ShfVal(L, "*Ty ?Req ?ZLen TxtSz VTxt Dft VRul Expr"), _
                     Ty, .Req, .ZLen, .TxtSz, .VTxt, .Dft, .VRul, .Expr
    .Ty = DaoTy(Ty)
    If L <> "" Then
        Push OEr, ErMsg_ExcessEleItm(ELin.Ix, L)
    End If
    If Ty = 0 Then
...
Select Case True
Case Tbl = Fld:       Set FndFd = NewIdFd(Fld)
Case AyHas(Tny, Tbl): Set FndFd = NewFkFd(Fld)
Case Else
