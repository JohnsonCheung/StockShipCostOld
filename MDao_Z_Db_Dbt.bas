...
Function DbtChkCol(A As Database, T, LnkColVbl$) As String()
...
If ActTy <> Ty Then
    DbtChkFldType = FmtQQ("Table[?] field[?] should have type[?], but now it has type[?]", T, F, ShtDaoTy(Ty), ShtDaoTy(ActTy))
End If
End Function
...
With FF
    DbtFFdInfDr = Array(F, IIf(DbtfIsPk(A, T, F), "*", ""), DaoTyStr(.Type), .Size, .DefaultValue, .Required, FdDes(FF))
End With
End Function
...
Sub DbtReStru(A As Database, Stru, EF As EF)
...
Function DbtSq(A As Database, T, Optional InclFldNm As Boolean) As Variant()
DbtSq = RsSq(DbtRs(A, T), InclFldNm)
End Function
...
Private Sub ZZ_DbtCrtDupKeyRecTbl()
TblDrp "#A #B"
...
Private Sub ZZ_DbtSpecReSeqFld()
...
ZZ:
    Dim A As Database
    Set A = FbDb(SampleFb_Duty_Dta)
    Dim Dr(), Dry(), T
    For Each T In DbTny(A)
        Erase Dr
        Push Dr, T
        PushIAy Dr, DbtPk(A, T)
        PushI Dry, Dr
    Next
    DryBrw Dry
    Exit Sub
