...
Function SrcBdyFmCnt(A$()) As FmCnt
Dim Lno&
Dim Cnt&
   Lno = SrcDclLinCnt(A) + 1
   Cnt = Sz(A) - Lno + 1
Set SrcBdyFmCnt = FmCnt(Lno, Cnt)
End Function
...
Function SrcHasMth(A$(), MthNm, Optional WhMdy$, Optional WhKd$) As Boolean
Dim O&: O = SrcMthNmIx(A, MthNm)
If O < 0 Then Exit Function
Dim Mdy$, Ty$, Nm$
LinMthNmBrkAsg A(O), Mdy, Ty, Nm
SrcHasMth = True
'If MdyIsSel(Mdy, WhMdy) Then Exit Function
'If KdIsSel(MthTyKd(Ty), WhMthKd) Then Exit Function
SrcHasMth = False
End Function
...
Dim FTIx() As FTIx: FTIx = SrcMthNmFTIxAy(A, MthNm)
Dim O$(), J%
For J = 0 To UB(FTIx)
...
Private Sub Z_SrcMthBdyLy()
Dim MthNm$: MthNm = "A"
Dim Act$()
Act = SrcMthBdyLy(CurSrc, MthNm)
End Sub

Function SrcMthFmCntAy(A$(), MthNm) As FmCnt()
...
For J = 0 To UB(FmAy)
   ...
   FmCnt = FTIxFmCnt(FTIx)
   PushObj O, FmCnt
Next
SrcMthFmCntAy = O
End Function

Sub SrcMthDrAsg(A, OMdy$, OTy$, ONm$, OPrm$, ORet$, OLinRmk$, OLines$, OTopRmk$)
...
Function SrcMthBrkAy(A$(), Optional B As WhMth) As Variant()
Dim L
For Each L In AyNz(A)
    PushNonZSz SrcMthBrkAy, LinMthNmBrk(CStr(L), B)
Next
End Function
...
Dim FTIxAy() As FTIx
   FTIxAy = SrcMthNmFTIxAy(A, MthNm)
Dim O$()
   O = A
   Dim J%
   For J = UB(FTIxAy) To 0 Step -1
       O = AyExlFTIx(O, FTIxAy(J))
   Next
SrcRmvMth = O
End Function

Function SrcRmvTy(A$(), TyNm$) As String()
SrcRmvTy = AyExlFTIx(A, DclTyFTIx(A, TyNm))
End Function
