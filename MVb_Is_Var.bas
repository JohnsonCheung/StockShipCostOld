...
End Function
Sub IsEqAss(A, B)
If VarType(A) <> VarType(B) Then MsgWhStop "Type Diff", "A-Ty B-Ty", TypeName(A), TypeName(B)
Select Case True
Case IsArray(A): AyIsEqAss A, B
Case IsObject(A): If ObjPtr(A) <> ObjPtr(B) Then MsgWhStop "Two object are diff", "A-&-B-TypeName", TypeName(A)
End Select
End Sub
...
Select Case True
Case IsArray(A): IsEq = AyIsEq(A, B)
...
Dim AK(): AK = A.Keys
If Not AyIsEq(AK, B.Keys) Then Exit Function
Dim K
For Each K In AK
...
IsInLikss = IsInLikAy(A, SslSy(Likss))
End Function
